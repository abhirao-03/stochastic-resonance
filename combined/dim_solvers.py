...
class solver():
    ...
    def langevin_exact(self):
        ...
        # Initialize array for all trajectories
        x = jnp.zeros((self.sde.num_steps, self.sde.num_trajectories))
        x = x.at[0].set(self.sde.x_init)

        # Initialize integral term for all trajectories
        integral_term = jnp.zeros((self.sde.num_steps, self.sde.num_trajectories))
        
        # Calculate integral term for all trajectories simultaneously
        for i in range(self.sde.num_steps - 1):
            t = self.sde.time_vec[i]
            integral_term = integral_term.at[i+1].set(
                integral_term[i] + jnp.exp(self.sde.theta*t) * self.sde.noise[i]
            )

        # Calculate solution for all trajectories simultaneously
        for i in range(self.sde.num_steps - 1):
            t = self.sde.time_vec[i]
            initial_term = self.sde.x_init * jnp.exp(self.sde.theta * t)
            drift_term = self.sde.MU * (1 - jnp.exp(self.sde.theta * t))
    ...
    def gbm_exact(self):
        ...
        for i in range(self.sde.num_steps - 1):
            t = self.sde.time_vec[i]
            x = x.at[i+1].set(
                self.sde.x_init * jnp.exp(
                    (self.sde.MU - (self.sde.SIGMA**2)/2)*t + self.sde.SIGMA * self.sde.noise[i]
                )
            )
