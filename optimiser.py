...
# def simulate(jump_mult: int, noise,  delta=100):
...
def run(jump_mult):
    ...
    valid_jump_times = jump_times[~np.isnan(jump_times)]
    if len(valid_jump_times) == 0:
        return np.nan  # Return NaN if all trajectories are invalid
